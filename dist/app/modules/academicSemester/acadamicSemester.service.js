...
const acadamicSemester_model_1 = __importDefault(require("./acadamicSemester.model"));
...
// get all semester
const getAllSemesterService = (filters, pagination) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    //   },
    // ];
    const { page, limit, skip, sortBy, sortOrder } = paginationHelpers_1.paginationHelper.paginationCalculation(pagination);
    const sortConditions = {};
    if (sortBy && sortOrder) {
        sortConditions[sortBy] = sortOrder;
    }
    const whereConditions = andConditions.length ? { $and: andConditions } : {};
    const result = yield acadamicSemester_model_1.default.find(whereConditions)
        .sort(sortConditions)
        .limit(limit)
        .skip(skip);
...
const updateSemesterService = (id, payload) => __awaiter(void 0, void 0, void 0, function* () {
    if (payload.title &&
        payload.code &&
    ...
    const result = yield acadamicSemester_model_1.default.findOneAndUpdate({ _id: id }, payload, {
        new: true,
    });
    return result;
});
const deleteSingleSemesterService = (id, payload) => {
    const finallyDeleteData = acadamicSemester_model_1.default.findOneAndDelete({ _id: id }, payload);
